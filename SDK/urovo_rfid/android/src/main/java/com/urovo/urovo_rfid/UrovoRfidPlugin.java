...
public class UrovoRfidPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler {
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
        ...
        //
        usdkManager = USDKManager.getInstance();
    ...
    private void initEventSink(FlutterPluginBinding flutterPluginBinding) {
        log("initEventSink");
        log("==DeviceId==" + new DeviceManager().getDeviceId());
        // 初始事件
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        } else if (method.equals("enableScanHead")) {
    ...
    private void killbyTID(MethodCall call, Result result) {
        ...
        String btAryPassWord = call.argument("btAryPassWord");
        if (tidlen == null) {
            result.success(ERR_CODE);
            return;
        }
        int ret = rfidManager.killTagByTid(tidlen, tid, btAryPassWord);
        result.success(ret);
    }
    ...
    private void lockbyTID(MethodCall call, Result result) {
        ...
        int ret = rfidManager.lockbyTID(tidlen, tid, btMemBank,
                btLockType, btAryPassWord);
        result.success(ret);
    }
    ...
    private void readTag(MethodCall call, Result result) {
        ...
        String pwArr = call.argument("pwArr");
        if (memBank == null || wordAdd == null || wordCnt == null) {
            result.success(ERR_CODE + "");
            return;
        }
        String ret = rfidManager.readTag(epc, memBank,
                wordAdd, wordCnt, pwArr);
        result.success(ret);
    }
    ...
    private void readDataByTid(MethodCall call, Result result) {
        ...
        String ret = rfidManager.readDataByTid(tidStr, mem, wordPtr,
                num, password);
        result.success(ret);
    }
...
//        Integer btWordCnt = call.argument("btWordCnt");
        String btAryData = call.argument("btAryData");
        if (btMemBank == null || btWordAdd == null
//                || btWordCnt == null
        ) {
            result.success(ERR_CODE);
            return;
        }
        int ret = rfidManager.writeTag(epc, btAryPassWord, btMemBank,
                btWordAdd, btAryData);
        result.success(ret);
    }
    ...
    private void writeTagByTid(MethodCall call, Result result) {
        ...
        int ret = rfidManager.writeTagByTid(tidStr, mem, wordPtr, password, data);
        result.success(ret);
    }
    ...
    private void WriteEPC(MethodCall call, Result result) {
        ...
        int ret = rfidManager.writeEpc(epcLen, epc, password);
        result.success(ret);
    }
    ...
    private void lockTag(MethodCall call, Result result) {
        ...
        int ret = rfidManager.lockTag(epc, btAryPassWord, btMemBank, btLockType);
        result.success(ret);
    }
    ...
    private void setOutputPower(MethodCall call, Result result) {
        ...
        int ret = rfidManager.setOutputPower(power);
        result.success(ret);
    }
    ...
    private void setFrequencyRegion(MethodCall call, Result result) {
        ...
        int ret = rfidManager.setFrequencyRegion(btRegion, btStartRegion, btEndRegion);
        result.success(ret);
    }
    ...
    private void setCustomRegion(MethodCall call, Result result) {
        ...
        int ret = rfidManager.setCustomRegion((flags == null ? 0 : flags),
    ...
    private void setProfile(MethodCall call, Result result) {
        ...
        int ret = rfidManager.setProfile(arg);
        result.success(ret);
    }
    ...
    private void iso180006BInventory(Result result) {
        ArrayList<Tag6B> tag6BS = rfidManager.iso180006BInventory();
        result.success(new Gson().toJson(tag6BS));
    }

    private void iso180006BLockTag(MethodCall call, Result result) {
        String btAryUID = call.argument("btAryUID");
        Integer btWordAdd = call.argument("btWordAdd");
        int ret = rfidManager.iso180006BLockTag(btAryUID,
                (btWordAdd == null ? 0 : btWordAdd));
        result.success(ret);
    }
    ...
    private void iso180006BQueryLockTag(MethodCall call, Result result) {
        String btAryUID = call.argument("btAryUID");
        Integer btWordAdd = call.argument("btWordAdd");
        Integer lockState = call.argument("lockState");
        int ret = rfidManager.iso180006BQueryLockTag(btAryUID,
                (btWordAdd == null ? 0 : btWordAdd), lockState);
        result.success(ret);
    }
    ...
    private void iso180006BReadTag(MethodCall call, Result result) {
        ...
        Integer btWordAdd = call.argument("btWordAdd");
        Integer btWordCnt = call.argument("btWordCnt");
        int ret = Integer.parseInt(rfidManager.iso180006BReadTag(btAryUID,
                (btWordAdd == null ? 0 : btWordAdd),
                (btWordCnt == null ? 0 : btWordCnt)));
        result.success(ret);
    }
    ...
    private void iso180006BWriteTag(MethodCall call, Result result) {
        ...
        Integer btWordAdd = call.argument("btWordAdd");
        Integer btWordCnt = call.argument("btWordCnt");
        String btAryBuffer = call.argument("btAryBuffer");
    ...
    private void startInventory(MethodCall call, Result result) {
        ...
        int startInventory = rfidManager.startInventory((session == null ? 0 : session));
    ...
    private void enableScanHead(MethodCall call, Result result) {
        ...
        usdkManager.enableScanHead(isOpen);
    }
    ...
    private void init() {
        log("doInit");
        usdkManager.init(new InitListener() {
            @Override
